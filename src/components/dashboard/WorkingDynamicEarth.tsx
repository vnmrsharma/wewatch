...
interface WorkingDynamicEarthProps {
...
const WorkingDynamicEarth: React.FC<WorkingDynamicEarthProps> = ({ 
  location, 
  onLayerChange 
}) => {
  ...
  const loadSatelliteImagery = async () => {
    ...
    try {
      // Generate NASA GIBS URL for the location
      ...
      if (location.lat >= 20 && location.lat <= 35 && location.lng >= 70 && location.lng <= 90) {
        ...
        // Default to working tile
        tileX = 23;
        tileY = 24;
      }

      // Select layer based on selection
      let layerId = 'MODIS_Terra_CorrectedReflectance_TrueColor';
      switch (selectedLayer) {
        case 'temperature':
          layerId = 'MODIS_Terra_Land_Surface_Temperature_Day';
          break;
        case 'vegetation':
          layerId = 'MODIS_Terra_NDVI';
          break;
        case 'airQuality':
      ...
      const imageUrl = `https://gibs.earthdata.nasa.gov/wmts/epsg4326/best/${layerId}/default/${date}/250m/${zoom}/${tileX}/${tileY}.jpg`;
  ...
  const loadNaturalEvents = async () => {
    try {
      // Mock natural events for demonstration
      const mockEvents = [
        {
          id: '1',
          title: 'Wildfire Alert',
          categories: [{ title: 'Wildfires' }],
          description: 'Active wildfire detected in the region'
        },
        {
          id: '2',
          title: 'Storm Warning',
  ...
  const loadEnvironmentalData = async () => {
    try {
      ...
      const baseTemp = 30 - (Math.abs(location.lat) / 90) * 40; // Temperature based on latitude
      const season = getCurrentSeason();
      const seasonAdjustment = season === 'winter' ? -10 : season === 'summer' ? 10 : 0;
      
      const data = {
        temperature: baseTemp + seasonAdjustment + (Math.random() - 0.5) * 10,
    ...
    <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
      ...
      <div className="relative mb-4">
        <div className="relative w-full h-80 bg-gradient-to-b from-blue-900 to-blue-700 rounded-lg overflow-hidden">
          ...
          {currentImageUrl && !imageError && imageLoaded && (
          ...
          {imageError && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-white text-center">
                ...
                <p className="text-xs mt-1">Using Earth visualization</p>
              </div>
            </div>
          )}
      ...
      <div className="bg-blue-50 rounded-lg p-4 mb-4">
        ...
        <p className="text-sm text-blue-800">
          Dynamic Earth visualization for {location.city}, {location.country} shows real-time satellite data 
          with {selectedLayer} layer analysis. The revolving Earth provides an interactive view of environmental 
          conditions with location tracking and natural event monitoring.
        </p>
      </div>
...
export default WorkingDynamicEarth;
